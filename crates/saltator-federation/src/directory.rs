...
fn public_chunk(rooms: &RoomServer, room_id: &str) -> Result<Option<serde_json::Value>, String> {
    let store = rooms.store();
    let Some(meta) = store.meta(room_id).map_err(|e| e.to_string())? else {
        return Ok(None);
    };
    let current = store
        .resolve_group(room_id, meta.current_group)
        .map_err(|e| e.to_string())?;
    let content_of = |ty: &str| -> Result<Option<serde_json::Value>, String> {
        let Some(event_id) = current.get(&(ty.to_owned(), String::new())) else {
            return Ok(None);
        };
        let Some(stored) = store.event(event_id).map_err(|e| e.to_string())? else {
            return Ok(None);
        };
    ...
    let mut chunk = serde_json::json!({
        ...
        "world_readable": str_field(&content_of("m.room.history_visibility")?, "history_visibility")
            .as_deref()
            == Some("world_readable"),
        "guest_can_join": str_field(&content_of("m.room.guest_access")?, "guest_access").as_deref()
            == Some("can_join"),
        "join_rule": str_field(&content_of("m.room.join_rules")?, "join_rule")
            .unwrap_or_else(|| "public".to_owned()),
    });
    let obj = chunk.as_object_mut().expect("chunk is an object");
    if let Some(name) = str_field(&content_of("m.room.name")?, "name") {
        obj.insert("name".to_owned(), name.into());
    }
    if let Some(topic) = str_field(&content_of("m.room.topic")?, "topic") {
        obj.insert("topic".to_owned(), topic.into());
    }
    if let Some(alias) = str_field(&content_of("m.room.canonical_alias")?, "alias") {
        obj.insert("canonical_alias".to_owned(), alias.into());
    }
    if let Some(url) = str_field(&content_of("m.room.avatar")?, "url") {
...
fn serve(state: &FedState, search: Option<&str>, limit: Option<u64>) -> FedResult {
...
pub async fn public_rooms_post(
    ...
    let search = body
        ...
        .and_then(|v| v.as_str())
        .map(ToOwned::to_owned);
    serve(&state, search.as_deref(), limit)
}
